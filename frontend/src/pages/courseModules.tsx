...
const CourseModulesPage = () => {
  ...
  // 2. Safely pull completed metrics from your actual 'progress' slice
              ...
              >
                ...
                <ul className="divide-y divide-slate-100 dark:divide-slate-800/60">
                  {mod.videos?.map((video: any, vIdx: number) => {
                    // 1. UPDATED: Track completion based on the unique video database string ID
                    const lectureKey = `${courseId}-${mod.id}-${video.id}`;
                    const isLecturePassed = completedLectures.includes(lectureKey);

                    // 2. UPDATED: Sequential gate locks adapt smoothly using array indexes internally
                    let isLocked = false;
                    if (vIdx > 0) {
                    ...
                    return (
                      <li key={video.id}>
                        <button
                          disabled={isLocked}
                          // 3. FIXED & COMPLETED: Passes video.id directly into the application router route string 
